...
\begin{description}
\item[\texttt{n\_tplts [integer 0]:}] number of triplets to resolve energy transfer between modes
\item[\texttt{split\_em\_tplt [logical True]:}] output energy transfer involving $\phi$ and $A_{\parallel}$ seperately. Only applicable if n\_fields $\>$ 1.
\item[\texttt{n\_dfs\_tplts [integer 1]:}] calculate energy transfer between supplied distribution function structures (from svd or eigenvectors)
\item[\texttt{kx\_triplets [integer*3N 0]:}] $k_{x}$ index for fourier modes to calculate energy transfer between. Must be in k-k'=k'', eg 0 -2 2  1 -2 3 \ldots
\item[\texttt{ky\_triplets [integer*3N 0]:}] $k_{y}$ index for fourier modes to calculate energy transfer between. Needs $k_{y}>0$ Must be in k-k'=k'', eg 4 0 4  4 0 4 \ldots
\item[\texttt{SVD\_df\_file\_path [str './']:}] location of modes for use in the triplet diagnostic, and for dfout files for SVD diagnostic
\item[\texttt{num\_ky\_modes [integer 0]:}] number of independent wavenumbers to record for dfout diagnostic
\item[\texttt{num\_kx\_modes [integer 0]:}] number of connections in dfout files
\item[\texttt{which\_ky [integer*N 0]:}] $k_{y}$ index for dfout
\item[\texttt{which\_kx\_center [integer*N 0]:}] central $k_{x}$ index for dfout
\item[\texttt{SVD\_proj [logical False]:}] enables SVD projection routine
\item[\texttt{SVD\_f0\_flag [logical True]:}] normalizes inner product for SVD with F0 to be more like free energy
\item[\texttt{SVD\_kx\_ind [integer*N 0]:}] $k_{x}$ wavenumbers index for SVD
\item[\texttt{SVD\_ky\_ind [integer*N 0]:}] $k_{y}$ wavenumbers index for SVD
\item[\texttt{SVD\_nkx0 [integer*N 0]:}] number of parallel connections for SVD
\item[\texttt{n\_svd\_ind [integer*N 0]:}] how many SVDs to calculate
\item[\texttt{SVD\_df\_n\_time [integer*N 0]:}] number of timesteps to use
\item[\texttt{SVD\_start\_time [real*N 0]:}] time to start from dfout files
\item[\texttt{SVD\_sparse\_factor [integer*N 0]:}] skip every SVD\_sparse\_factor
\item[\texttt{SVD\_df\_file\_suffix [string*N 0]:}] e.g. files named '*\_1' '*\_2' \ldots
\item[\texttt{SVD\_n\_restarts [integer*N 0]:}] number of files to use dfouts from
\end{description}
...
\bea
k_y\rho_{\rm ref} = n\,\frac{\tt Cy}{\tt Lref} c_{\rm ref} \frac{m_{\rm ref} c}{e\,{\tt Bref}}
\eea
...
\bibitem{GENE-webpage} F.~{Jenko} and {The GENE development team}, \href{http://genecode.org}{The
  {GENE} code}.
...
\bibitem{LapillonnePhD} X.~Lapillonne, {\em Local and Global Eulerian Gyrokinetic Simulations of Microturbulence in Realistic Geometry with Applications to the TCV Tokamak}, PhD thesis, EPFL Lausanne, 2010 (available at gene.rzg.mpg.de)
\bibitem{ToldPhD} D.~Told, {\em Gyrokinetic Microturbulence in Transport Barriers}, Universit\"at Ulm, 2012 (available at genecode.org)
\bibitem{MorelPoP11} P.~Morel {\em et al.}, Phys.~Plasmas \textbf{18}, 072301 (2011); \url{http://dx.doi.org/10.1063/1.3601053}
\bibitem{BanonPhD} A.~Ba\~{n}{\'o}n Navarro, {\em Gyrokinetic Large Eddy Simulations}, Universit\'e Libre de Bruxelles, 2012 (available at genecode.org)
\bibitem{BanonPoP14} A.~Ba\~{n}{\'o}n Navarro {\em et al.}, Phys.~Plasmas {\bf 21}, 032304 (2014)
\end{thebibliography}
\end{document}
